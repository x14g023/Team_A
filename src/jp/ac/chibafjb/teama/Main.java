...
public class Main extends HttpServlet {
	...
	public void init() throws ServletException {
		...
		try {
			...
			// テーブルが無ければ作成
			if (!mOracle.isTable("Table_Content")
					&& mOracle.isTable("Table_Genre")) {
	...
	public String getGenreName(int param) throws SQLException {

		ResultSet gename = mOracle
				.query("select Genre_Name from Table_Genre where GenreID = "
						+ param);

		String name = null;
		while (gename.next()) {
			name = gename.getString(1);
			if (name == null) {
				name = null;
			}
		}
	...
	protected void action(HttpServletRequest request,
		...
		int gparam = Integer.parseInt(param3);

		String param4 = request.getParameter("k");
		if (param4 == null) {
			param4 = "1";
		}
		...
		if (param1 != null && param1.length() > 0 && param2 != null
			...
			String sql = String
					.format("insert into Table_Content values(Table_Content_SeqID.nextval,'%s','%s',SYSDATE,%d)",
							Oracle.STR(datan), Oracle.STR(dataw), gparam);
			// デバッグ用
		...
		//ジャンル追加
			String genreInsert = request.getParameter("genreName");
			String password = request.getParameter("pass");
			if (genreInsert != null && genreInsert.length() > 0
					&& password != null) {

				String pass = new String(password.getBytes("ISO-8859-1"),
						"UTF-8");
				if (pass.equals(Main.password)) {
					String genreName = new String(
							genreInsert.getBytes("ISO-8859-1"), "UTF-8");
					String sql = String
							.format("insert into Table_Genre values(Table_Content_SeqID.nextval,'%s')",
									genreName);
					// デバッグ用
					System.out.println("DEBUG:SQL文 " + sql);
					// DBにSQL文を実行させる
		...
		StringBuilder gdele = new StringBuilder();

		// データの抽出
		try {
			ResultSet res = mOracle
					.query("select * from Table_Content WHERE genreID = "
							+ gparam + "ORDER BY time desc");
			...
			while (res.next()) {
				...
				if (name != null && write != null) {
					...
					sb.append(String.format(
							...
							CONVERT(name), cal.get(Calendar.YEAR),
			...
			while (genre.next()) {
				...
				if (gname != null) {
					// 文字列バッファにメッセージ内容を貯める
					// CONVERTはタグの無効化
					gn.append(String.format("<a href=\"?j=" + gid
							+ "\">%s</a><br>", gname));
				}
			}
			...
			while (genre.next()) {
				...
				if (gname != null) {
					// 文字列バッファにメッセージ内容を貯める
					// CONVERTはタグの無効化
					gdele.append(String.format("<td>"
							+ "<input type=\"radio\" name=\"hyouka\" value=\""
							+ gid + "\"></td>" + "<td>%s</td>", gname));
				}
			}

			String gname = getGenreName(gparam);

			// メッセージの置換
			ts.replace("$(MSG)", sb.toString());
			ts.replace("$(GENRE)", gn.toString());
			ts.replace("$(GNAME)", gname);
			ts.replace("$(ID)", param3);
